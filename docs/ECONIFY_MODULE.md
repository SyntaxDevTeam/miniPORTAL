...
lub administrator serwera łączy swoje konto samodzielnie przez `/econify/servers`.
...
Pokazuje tylko tenanty, które bot zgłosił już do `/api/econify/guilds`. Flow
zaproszenia i połączenia konta właściciela/admina Discord znajduje się w widoku
`/econify/servers`, bo wymaga realnych uprawnień konkretnego użytkownika Discord.
...
Discord Developer Portal dla `ECONIFY_DISCORD_CLIENT_ID`. Jeśli użytkownik widzi
np. robocze `NewMain`, to trzeba zmienić nazwę, ikonę i opis tej aplikacji po
stronie Discorda albo wskazać w `.env` właściwą, dedykowaną aplikację Econify.
