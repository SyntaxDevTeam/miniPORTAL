    ...
    steps:
      ...
      - name: Publish artifact to BuildExplorer
        ...
        run: |
          ...
          jar="$(find "${BUILD_JAR_DIR}" -type f -name '*.jar' ! -name '*-sources.jar' ! -name '*-javadoc.jar' | sort | tail -n 1)"
          if [ -z "${jar}" ]; then
          ...
          filename="${BUILD_SLUG}-${BUILD_SERVER}-${version}-${BUILD_CHANNEL^^}-${GITHUB_RUN_NUMBER_VALUE}.jar"
