...
use SyntaxDevTeam\Cms\Core\ModuleUpdatePublisher;
use SyntaxDevTeam\Cms\Core\ModuleUpdateRepository;
...
final class SystemAdminModule implements ModuleInterface
{
    public function __construct(
        ...
        private readonly ModuleUpdateRepository $moduleUpdates,
        private readonly ModuleUpdatePublisher $moduleUpdatePublisher,
    ) {
    }
    ...
    public function registerRoutes(Router $router): void
    {
        ...
        $router->get('/api/module-updates/catalog', fn () => $this->serveModuleUpdateCatalog());
        $router->get(
            '/api/module-updates/{filename}',
            fn (Request $request) => $this->serveModuleUpdateArchive($request)
        );
        ...
        $router->post('/admin/modules/check-updates', fn (Request $request) => $this->guard(
            $request,
            'modules.view',
            fn () => $this->checkModuleUpdates($request)
        ));
        $router->post('/admin/modules/remote-update', fn (Request $request) => $this->guard(
            $request,
            'modules.install',
            fn () => $this->applyRemoteModuleUpdate($request)
        ));
    ...
    private function renderDashboard(): void
    {
        ...
        try {
            ...
            if ($latestRelease !== null) {
                echo '<div class="alert alert-warning admin-update-notice" role="status">';
                echo '<p class="mb-0">Dostępna jest aktualizacja miniPORTAL '
                    . htmlspecialchars((string) $latestRelease['version'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8')
                    . '. Otwórz sekcję Aktualizacje, aby poznać listę zmian.</p>';
                echo '<a class="btn btn-primary" href="index.php?route=/admin/system-updates">Sprawdź aktualizację</a>';
                echo '</div>';
            }
        ...
        try {
            $remoteModuleUpdates = $this->availableRemoteModuleUpdates($moduleEntries);
            if ($remoteModuleUpdates !== []) {
                echo '<div class="alert alert-warning admin-update-notice" role="status">';
                echo '<p class="mb-0">Dostępne aktualizacje modułów: '
                    . count($remoteModuleUpdates)
                    . '. Manager modułów pokaże wersje i zgodność pakietów.</p>';
                echo '<a class="btn btn-primary" href="index.php?route=/admin/modules">Sprawdź moduły</a>';
                echo '</div>';
            }
        } catch (\Throwable $exception) {
            $this->theme->render_alert(
                'Nie można sprawdzić aktualizacji modułów: ' . $exception->getMessage(),
                'warning'
            );
        }
    ...
    private function renderPlatformUpdates(string $message = '', string $variant = 'info'): void
    {
        ...
        try {
            ...
            if ($releases === []) {
                ...
                $this->renderReleaseChangelog($latest['changelog']);
                if ($permissionIssues === []) {
            ...
            if ($this->isOwner() && $this->platformReleasePublisher->available()) {
                $publishPermissionIssues = $this->platformReleasePublisher->permissionIssues();
                $this->theme->start_admin_panel(
                    'Opublikuj własne wydanie',
                    'Instalacja macierzysta / Owner'
                );
                if ($publishPermissionIssues !== []) {
                    $this->theme->render_alert(
                        "Publikowanie wydań wymaga przygotowania praw zapisu. Brak dostępu:\n"
                        . implode("\n", array_map(static fn (string $issue): string => '- ' . $issue, $publishPermissionIssues))
                        . "\n\nWklej na serwerze:\n" . $this->platformReleasePublisher->remediationCommand(),
                        'warning'
                    );
                }
                $this->theme->render_alert(
                ...
                $this->theme->render_form(
                    ...
                    [
                        ...
                        [
                            ...
                            'label' => 'Lista zmian',
                            'type' => 'richtext',
                            'value' => "- Dodano nową funkcję.\n- Naprawiono błąd aktualizacji.\n- Poprawiono bezpieczeństwo.",
                            'format_name' => 'changelog_format',
                            'format_value' => 'markdown',
                            'help' => 'Markdown w stylu GitHub. Najlepiej użyj listy punktowanej; do katalogu wydań trafi czysta lista maksymalnie 50 pozycji.',
                        ],
                    ],
     ...
     */
    private function renderReleaseChangelog(array $items): void
    {
        echo '<div class="admin-release-changelog"><div class="admin-release-changelog-label">Lista zmian</div><ul>';
    ...
    private function servePlatformReleaseArchive(Request $request): void
    {
        ...
        try {
            ...
            DownloadResponse::send($path, $release['filename'], 'application/zip', false, [
                'Cache-Control' => 'public, max-age=86400, immutable',
                'X-Release-SHA256' => (string) $release['checksum'],
            ]);
        } catch (\Throwable) {
            http_response_code(503);
        }
    ...
    private function serveModuleUpdateCatalog(): void
    {
        if (!$this->moduleUpdatePublisher->available()) {
            http_response_code(404);
            echo '{"modules":[]}';
            return;
        }
        try {
            $payload = json_encode(
                [
                    ...
                    'modules' => $this->moduleUpdatePublisher->catalog(),
                ],
                JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR
            ) . PHP_EOL;
            if (!headers_sent()) {
            ...
            error_log('[miniPORTAL module repository] ' . $exception->getMessage());
            http_response_code(503);
            echo '{"modules":[]}';
        }
    }

    private function serveModuleUpdateArchive(Request $request): void
    {
        if (!$this->moduleUpdatePublisher->available()) {
            http_response_code(404);
            return;
        ...
        $filename = $request->routeString('filename');
        try {
            $entry = $this->moduleUpdatePublisher->findByFilename($filename);
            if ($entry === null) {
                http_response_code(404);
                return;
            }
            DownloadResponse::send(
                $this->moduleUpdatePublisher->archivePath($entry),
                $entry['filename'],
                'application/zip',
                false,
                [
                    'Cache-Control' => 'public, max-age=86400, immutable',
                    'X-Module-SHA256' => (string) $entry['checksum'],
                ]
            );
        } catch (\Throwable $exception) {
            error_log('[miniPORTAL module repository] ' . $exception->getMessage());
    ...
    private function publishPlatformRelease(Request $request): void
    {
        ...
        try {
            ...
            $changelog = self::normalizeReleaseChangelog(
                $request->postString('changelog'),
                $request->postString('changelog_format', 'markdown')
            );
            ...
            $this->audit->record(
                ...
                'success',
                $version . ' / entries:' . count($changelog),
                $actor?->id
            );
     ...
     * @return list<string>
     */
    private static function normalizeReleaseChangelog(string $source, string $format): array
    {
        $source = self::normalizeReleaseText($source);
        $format = strtolower(trim($format));
        $lines = $format === 'html'
            ? self::releaseChangelogLinesFromHtml($source)
            : self::releaseChangelogLinesFromMarkdown($source);
        $items = [];
        foreach ($lines ?: [] as $line) {
            $item = self::plainReleaseChangelogItem((string) $line);
            if ($item !== '') {
                $items[] = $item;
            }
        }
     ...
     * @return list<string>
     */
    private static function releaseChangelogLinesFromMarkdown(string $source): array
    {
        $lines = preg_split('/\R/u', str_replace("\t", '    ', $source)) ?: [];
        $fallback = [];
        $listItems = [];
        $current = null;
        foreach ($lines as $line) {
            $trimmed = trim($line);
            if ($trimmed === '' || preg_match('/^\|?\s*:?-{3,}:?\s*(?:\|\s*:?-{3,}:?\s*)+\|?$/u', $trimmed) === 1) {
                continue;
            }
            if (preg_match('/^ {0,3}(?:[-*+]|\d+[.)])\s+(.+)$/u', $line, $match) === 1) {
                if ($current !== null) {
                    $listItems[] = $current;
                }
                $current = $match[1];
            ...
            if ($current !== null && preg_match('/^ {2,}\S/u', $line) === 1) {
                $current .= ' ' . $trimmed;
                continue;
            }
            if ($current !== null) {
        ...
        return $listItems !== [] ? $listItems : $fallback;
    }

    /**
     * @return list<string>
     */
    private static function releaseChangelogLinesFromHtml(string $source): array
    {
        $lines = [];
        if (preg_match_all('#<li\b[^>]*>(.*?)</li>#is', $source, $matches) > 0) {
        ...
        $text = preg_replace('#<(br|/p|/div|/h[1-6]|/blockquote)\b[^>]*>#i', "\n", $source) ?? $source;

        return preg_split('/\R/u', $text) ?: [];
    }

    private static function plainReleaseChangelogItem(string $line): string
    {
        $line = self::normalizeReleaseText($line);
        $line = html_entity_decode(strip_tags($line), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');
        $line = preg_replace('/^\s*(?:[-*+]|\d+[.)])\s+/u', '', $line) ?? $line;
        $line = preg_replace('/^\s*>\s*/u', '', $line) ?? $line;
        $line = preg_replace('/^\s*#{1,6}\s*/u', '', $line) ?? $line;
        $line = preg_replace('/^\s*\[[ xX]\]\s*/u', '', $line) ?? $line;
        $line = preg_replace('/!\[([^\]]*)\]\([^)]+\)/u', '$1', $line) ?? $line;
        $line = preg_replace('/\[([^\]]+)\]\([^)]+\)/u', '$1', $line) ?? $line;
        $line = preg_replace('/`([^`]+)`/u', '$1', $line) ?? $line;
        $line = preg_replace('/(\*\*|__)(.*?)\1/u', '$2', $line) ?? $line;
        $line = preg_replace('/\*(.*?)\*/u', '$1', $line) ?? $line;
        $line = preg_replace('/~~(.*?)~~/u', '$1', $line) ?? $line;
        $line = preg_replace('/^\s*\|?\s*:?-{3,}:?\s*(?:\|\s*:?-{3,}:?\s*)+\|?\s*$/u', '', $line) ?? $line;
        $line = preg_replace('/\s+/u', ' ', $line) ?? $line;

        return trim($line, " \t\n\r\0\x0B|");
    }

    private static function normalizeReleaseText(string $value): string
    {
        ...
        return is_string($clean) && preg_match('//u', $clean) === 1 ? $clean : '';
    }
    ...
    private function renderModules(string $message = '', string $variant = 'info'): void
    {
        ...
        if ($publisherMode) {
            $this->theme->render_alert(
                'Tryb wydawniczy jest aktywny. Centralny katalog automatycznie publikuje podpisane pakiety po zmianie wersji lub zawartości modułu.',
                'success'
            );
        }
        ...
        $remoteUpdates = [];
        $remoteUpdateError = '';
        if ($this->moduleUpdates->configured()) {
            try {
                $remoteUpdates = $this->availableRemoteModuleUpdates($moduleEntries);
            } catch (\Throwable $exception) {
                $remoteUpdateError = $exception->getMessage();
            }
        }

        $this->theme->start_admin_panel('Centralny kanał aktualizacji modułów', $this->moduleUpdates->configured() ? 'Aktywny' : 'Nieskonfigurowany');
        if (!$this->moduleUpdates->configured()) {
            $this->theme->render_alert(
                'Ta instalacja nie ma ustawionego MODULE_UPDATE_CATALOG_URL. Ręczny import archiwów pozostaje dostępny.',
                'info'
            );
        } elseif ($remoteUpdateError !== '') {
            $this->theme->render_alert('Nie można odczytać katalogu modułów: ' . $remoteUpdateError, 'warning');
        } elseif ($remoteUpdates === []) {
            $this->theme->render_alert('Wszystkie zainstalowane moduły korzystają z najnowszych dostępnych wersji.', 'success');
        } else {
            $this->theme->render_alert(
                'Dostępne aktualizacje: ' . count($remoteUpdates) . '. Każdy pakiet zostanie pobrany do kwarantanny, zweryfikowany i wdrożony z rollbackiem kodu.',
                'warning'
            );
        }
        if ($this->moduleUpdates->configured()) {
            $this->theme->render_form(
                'index.php?route=/admin/modules/check-updates',
                [],
                'Sprawdź teraz',
                $this->security->csrfToken()
            );
        ...
        foreach ($moduleEntries as $entry) {
            ...
            $remoteUpdate = $manifest !== null ? ($remoteUpdates[$manifest->id] ?? null) : null;
            $remoteCompatible = is_array($remoteUpdate) && $this->moduleUpdates->compatibleWith(
                $remoteUpdate['miniportal_constraint'],
                (string) ($this->config['app']['version'] ?? '0.0.0')
            );
            $actions = [];
            ...
            if (
                ...
                && $state->isInstalled()
                && !$updateAvailable
                && is_array($remoteUpdate)
                && $remoteCompatible
                && $allows('modules.install')
            ) {
                $actions[] = [
                    'label' => 'Pobierz ' . $remoteUpdate['version'],
                    'action' => 'index.php?route=/admin/modules/remote-update',
                    'fields' => [
                        'module_id' => $manifest->id,
                        'version' => $remoteUpdate['version'],
                    ],
                    'variant' => 'primary',
                    'confirm' => 'Pobrać podpisany pakiet z centralnego repozytorium i zaktualizować moduł? Kod zostanie podmieniony atomowo, migracje wykonane kontrolowanie, a błąd przywróci poprzednią wersję.',
                ];
            }
            if (
                $loadable
            ...
            $rows[] = [
                'cells' => [
                    ...
                    $state->version . ' / ' . $manifest->version
                        . (is_array($remoteUpdate) ? ' / zdalna ' . $remoteUpdate['version'] : ''),
                    ...
                    is_array($remoteUpdate) && !$remoteCompatible
                        ? 'Wymaga miniPORTAL ' . ($remoteUpdate['miniportal_constraint'] !== '' ? $remoteUpdate['miniportal_constraint'] : 'w zgodnej wersji')
                        : ($state->dataPreserved
                        ? 'Dane zachowane'
                        ...
                        if ($approvable) {
                            $actions[] = [
                                ...
                                'confirm' => $manifest->protected
                                    ? 'Zaktualizować lub odświeżyć chroniony moduł? Kod zostanie podmieniony atomowo, migracje wykonane od razu, a błąd przywróci poprzednią wersję.'
                                    : 'Zatwierdzić pakiet? Nowy moduł zostanie przeniesiony do modules/, a istniejący zaktualizowany albo odświeżony atomowo wraz z migracjami.',
                            ];
                        }
    ...
    private function checkModuleUpdates(Request $request): void
    {
        $actor = $this->auth->user();
        if (!$this->security->validateCsrfToken($request->postString('_token'))) {
            $this->audit->record($request, 'module_update_check', 'invalid_csrf', null, $actor?->id);
            http_response_code(403);
        ...
        try {
            if (!$this->moduleUpdates->configured()) {
                throw new \RuntimeException('Centralny kanał aktualizacji modułów nie jest skonfigurowany.');
            }
            $entries = $this->moduleManager?->modules() ?? [];
            $updates = $this->availableRemoteModuleUpdates($entries, true);
            $this->audit->record(
                $request,
                'module_update_check',
                'success',
            ...
            $this->renderModules(
                $updates === []
                    ? 'Sprawdzono centralny katalog. Wszystkie moduły są aktualne.'
                    : 'Sprawdzono centralny katalog. Dostępne aktualizacje: ' . count($updates) . '.',
                $updates === [] ? 'success' : 'warning'
            );
        } catch (\Throwable $exception) {
            $this->audit->record($request, 'module_update_check', 'failed', null, $actor?->id);
            $this->renderModules($exception->getMessage(), 'danger');
        }
    }

    private function applyRemoteModuleUpdate(Request $request): void
    {
        $actor = $this->auth->user();
        $moduleId = $request->postString('module_id');
        $version = $request->postString('version');
        if (!$this->security->validateCsrfToken($request->postString('_token'))) {
            $this->audit->record($request, 'module_remote_update', 'invalid_csrf', $moduleId, $actor?->id);
            http_response_code(403);
            $this->renderModules('Nieprawidłowy lub wygasły token CSRF.', 'danger');
            return;
        }
        if (
            ...
            $this->audit->record($request, 'module_remote_update', 'invalid_module', $moduleId, $actor?->id);
            $this->renderModules('Nieprawidłowy moduł, wersja albo niedostępny manager.', 'danger');
            return;
        }

        try {
            $local = $this->moduleManager->manifests()[$moduleId] ?? null;
            if ($local === null) {
                throw new \RuntimeException('Nie znaleziono lokalnego modułu do aktualizacji.');
            }
            $remote = $this->moduleUpdates->find($moduleId, $version, true);
            if ($remote === null || !version_compare($remote['version'], $local->version, '>')) {
                throw new \RuntimeException('Centralny katalog nie zawiera nowszej wskazanej wersji modułu.');
            }
            if ($remote['type'] !== $local->type || $remote['protected'] !== $local->protected) {
                throw new \RuntimeException('Centralny katalog próbuje zmienić tożsamość modułu.');
            }
            $platformVersion = (string) ($this->config['app']['version'] ?? '0.0.0');
            if (!$this->moduleUpdates->compatibleWith($remote['miniportal_constraint'], $platformVersion)) {
                throw new \RuntimeException(
                    'Aktualizacja modułu wymaga miniPORTAL ' . $remote['miniportal_constraint']
                    . '; bieżąca wersja to ' . $platformVersion . '.'
                );
            }

            $archive = $this->moduleUpdates->archivePath($remote);
            $import = $this->moduleArchiveImporter->importFile($archive, $remote['filename']);
            $importedManifest = $import['manifest'];
            if (
                $importedManifest === null
                || $importedManifest->id !== $moduleId
                || $importedManifest->version !== $version
            ) {
                throw new \RuntimeException(
                    'Pobrany pakiet nie odpowiada wpisowi katalogu: ' . (string) ($import['error'] ?? 'niezgodny manifest')
                );
            }
            ...
            $this->audit->record(
                $request,
                'module_remote_update',
                'success',
                $moduleId . ':' . $local->version . '->' . $result['manifest']->version,
                $actor?->id
            );
            $this->renderModules(
                'Moduł ' . $result['manifest']->name . ' zaktualizowano z centralnego repozytorium do wersji '
                . $result['manifest']->version . '. Podpis, migracje i rollback kodu zostały obsłużone przez standardowy manager.',
                'success'
            );
        } catch (\Throwable $exception) {
            $this->audit->record(
    ...
    /**
     * @param list<array<string, mixed>> $moduleEntries
     * @return array<string, array<string, mixed>>
     */
    private function availableRemoteModuleUpdates(array $moduleEntries, bool $forceRefresh = false): array
    {
        if (!$this->moduleUpdates->configured()) {
            return [];
        }
        ...
        foreach ($moduleEntries as $entry) {
            ...
            if ($manifest !== null && $state?->isInstalled() === true) {
                $installed[$manifest->id] = $manifest;
            }
        }

        $updates = [];
        $catalog = $this->moduleUpdatePublisher->available()
            ? $this->moduleUpdatePublisher->catalog()
            : $this->moduleUpdates->all($forceRefresh);
        foreach ($catalog as $candidate) {
            $local = $installed[$candidate['id']] ?? null;
            if ($local === null || !version_compare($candidate['version'], $local->version, '>')) {
                continue;
            }
            if ($candidate['type'] !== $local->type || $candidate['protected'] !== $local->protected) {
                throw new \RuntimeException('Katalog zawiera niezgodną tożsamość modułu ' . $candidate['id'] . '.');
            }
            if (!isset($updates[$candidate['id']]) || version_compare(
                $candidate['version'],
                $updates[$candidate['id']]['version'],
                '>'
            )) {
                $updates[$candidate['id']] = $candidate;
            }
        }
            ...
            $this->renderModules(
                $result['operation'] === 'updated'
                    ...
                    : ($result['operation'] === 'refreshed'
                        ? 'Pakiet ' . $manifest->name . ' odświeżył kod modułu w tej samej wersji ' . $manifest->version . '.'
    ...
    private function exportModuleArchive(Request $request): void
    {
        ...
        try {
            ...
            DownloadResponse::send($export['path'], (string) $export['filename'], (string) $export['mime']);
            @unlink($export['path']);
            exit;
        } catch (\Throwable $exception) {
    ...
    private function renderSettings(string $message = '', string $variant = 'info'): void
    {
        ...
        $values = $this->settings->themeSettings([
            ...
            'public_faststats_enabled' => (string) ($app['public_faststats_enabled'] ?? '0'),
            'public_faststats_site_key' => (string) ($app['public_faststats_site_key'] ?? ''),
            'public_faststats_cookieless' => (string) ($app['public_faststats_cookieless'] ?? '1'),
            'public_faststats_web_vitals' => (string) ($app['public_faststats_web_vitals'] ?? '1'),
            'public_faststats_error_tracking' => (string) ($app['public_faststats_error_tracking'] ?? '1'),
            'public_faststats_session_replays' => (string) ($app['public_faststats_session_replays'] ?? '0'),
            'public_faststats_debug' => (string) ($app['public_faststats_debug'] ?? '0'),
        ]);
        ...
        $this->theme->start_admin_panel('FastStats Web Analytics', 'Statystyki strony');
        $this->theme->render_form(
            'index.php?route=/admin/settings',
            [
                ['name' => 'settings_scope', 'label' => 'Zakres ustawień', 'type' => 'hidden', 'value' => 'faststats'],
                [
                    'name' => 'public_faststats_enabled',
                    'label' => 'Włącz FastStats na publicznych stronach',
                    'type' => 'checkbox',
                    'checked' => $values['public_faststats_enabled'] === '1',
                    'help' => 'Integracja ładuje oficjalny SDK Web Analytics tylko na indeksowalnych widokach publicznych.',
                ],
                [
                    'name' => 'public_faststats_site_key',
                    'label' => 'FastStats Site Key',
                    'value' => $values['public_faststats_site_key'],
                    'maxlength' => 160,
                    'autocomplete' => 'off',
                    'help' => 'Publiczny klucz witryny z panelu FastStats Web Analytics.',
                ],
                [
                    'name' => 'public_faststats_cookieless',
                    'label' => 'Tryb cookieless',
                    'type' => 'checkbox',
                    'checked' => $values['public_faststats_cookieless'] === '1',
                ],
                [
                    'name' => 'public_faststats_web_vitals',
                    'label' => 'Mierz Web Vitals',
                    'type' => 'checkbox',
                    'checked' => $values['public_faststats_web_vitals'] === '1',
                ],
                [
                    'name' => 'public_faststats_error_tracking',
                    'label' => 'Rejestruj błędy JavaScript',
                    'type' => 'checkbox',
                    'checked' => $values['public_faststats_error_tracking'] === '1',
                ],
                [
                    'name' => 'public_faststats_session_replays',
                    'label' => 'Włącz nagrania sesji',
                    'type' => 'checkbox',
                    'checked' => $values['public_faststats_session_replays'] === '1',
                    'help' => 'Włączaj tylko po świadomej decyzji prywatnościowej dla danej instalacji.',
                ],
                [
                    'name' => 'public_faststats_debug',
                    'label' => 'Tryb debug',
                    'type' => 'checkbox',
                    'checked' => $values['public_faststats_debug'] === '1',
                ],
            ],
            'Zapisz FastStats',
            $this->security->csrfToken()
    ...
    private function saveSettings(Request $request): void
    {
        ...
        if (!$this->security->validateCsrfToken($request->postString('_token'))) {
            $this->recordSettingsAudit($request, 'invalid_csrf', $actor->id);
            http_response_code(403);
        ...
        try {
            ...
            } elseif ($scope === 'faststats') {
                $this->settings->saveFastStatsSettings(
                    [
                        'public_faststats_enabled' => $request->postBool('public_faststats_enabled') ? '1' : '0',
                        'public_faststats_site_key' => $request->postString('public_faststats_site_key'),
                        'public_faststats_cookieless' => $request->postBool('public_faststats_cookieless') ? '1' : '0',
                        'public_faststats_web_vitals' => $request->postBool('public_faststats_web_vitals') ? '1' : '0',
                        'public_faststats_error_tracking' => $request->postBool('public_faststats_error_tracking') ? '1' : '0',
                        'public_faststats_session_replays' => $request->postBool('public_faststats_session_replays') ? '1' : '0',
                        'public_faststats_debug' => $request->postBool('public_faststats_debug') ? '1' : '0',
                    ],
                    $actor->id
                );
            ...
            if (in_array($scope, ['theme', 'branding', 'seo', 'faststats', 'navigation', 'favicon'], true)) {
                $this->templateCache->invalidateTags(['theme', 'homepage']);
            }
            $this->recordSettingsAudit($request, 'success', $actor->id);
            header('Location: index.php?route=/admin/settings', true, 303);
        } catch (\Throwable $exception) {
            error_log('System settings update failed: ' . $exception->getMessage());
            $this->recordSettingsAudit($request, 'failed', $actor->id);
            $this->renderSettings($exception->getMessage(), 'danger');
        }
    }

    private function recordSettingsAudit(Request $request, string $result, int $actorId): void
    {
        try {
            $this->audit->record($request, 'system_settings_update', $result, null, $actorId);
        } catch (\Throwable $exception) {
            error_log('System settings audit failed: ' . $exception->getMessage());
        }
    }
    ...
    private function exportLogs(Request $request): void
    {
        ...
        try {
            ...
            if ($stream === false) {
                throw new \RuntimeException('Nie można utworzyć bufora eksportu.');
            }
            (new AuditCsvExporter())->write($stream, $events);
            rewind($stream);
            DownloadResponse::sendString(
                (string) stream_get_contents($stream),
                'miniportal-audit-' . gmdate('Ymd-His') . '.csv',
