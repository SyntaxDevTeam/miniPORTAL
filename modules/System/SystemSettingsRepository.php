...
final class SystemSettingsRepository
{
    ...
    public function themeSettings(array $defaults): array
    {
        $rows = $this->database->read(
            ...
            ['setting_key' => [
                ...
                'public_faststats_enabled',
                'public_faststats_site_key',
                'public_faststats_cookieless',
                'public_faststats_web_vitals',
                'public_faststats_error_tracking',
                'public_faststats_session_replays',
                'public_faststats_debug',
            ]]
        ) ?? [];
        foreach ($rows as $row) {
    ...
    /** @param array<string, string> $settings */
    public function saveFastStatsSettings(array $settings, int $actorId): void
    {
        $siteKey = trim((string) ($settings['public_faststats_site_key'] ?? ''));
        $enabled = $this->settingEnabled($settings, 'public_faststats_enabled');
        if ($siteKey !== '' && preg_match('/^[A-Za-z0-9._:-]{8,160}$/', $siteKey) !== 1) {
            throw new RuntimeException('FastStats Site Key ma nieprawidłowy format.');
        }
        if ($enabled && $siteKey === '') {
            throw new RuntimeException('Włączona integracja FastStats wymaga Site Key.');
        }

        $this->upsertSettings([
            'public_faststats_enabled' => $enabled ? '1' : '0',
            'public_faststats_site_key' => $siteKey,
            'public_faststats_cookieless' => $this->settingEnabled($settings, 'public_faststats_cookieless') ? '1' : '0',
            'public_faststats_web_vitals' => $this->settingEnabled($settings, 'public_faststats_web_vitals') ? '1' : '0',
            'public_faststats_error_tracking' => $this->settingEnabled($settings, 'public_faststats_error_tracking') ? '1' : '0',
            'public_faststats_session_replays' => $this->settingEnabled($settings, 'public_faststats_session_replays') ? '1' : '0',
            'public_faststats_debug' => $this->settingEnabled($settings, 'public_faststats_debug') ? '1' : '0',
        ], $actorId);
    }
    ...
    /** @param array<string, string> $settings */
    private function settingEnabled(array $settings, string $key): bool
    {
        return filter_var($settings[$key] ?? '0', FILTER_VALIDATE_BOOL);
    }
