...
use SyntaxDevTeam\Cms\Modules\MediaLibrary\MediaAsset;
use SyntaxDevTeam\Cms\Modules\MediaLibrary\MediaAssetRepository;

final class CorePagesModule implements ModuleInterface, SeoProviderInterface
{
    public function __construct(
        ...
        private readonly ?MediaAssetRepository $mediaAssets = null,
    ) {
    }
    ...
    public function registerSeo(SeoIndex $seo): void
    {
        ...
        foreach ($this->homepageSections->visible() as $section) {
            if ($homepageLastModified === null || $section->updatedAt > $homepageLastModified) {
                $homepageLastModified = $section->updatedAt;
            }
        }
        $seo->add(
            '/',
            'SyntaxDevTeam',
            'Minecraft plugins, Discord bots and modular tools from SyntaxDevTeam.',
        ...
        foreach ($this->pages->published() as $page) {
            ...
            $seo->add(
                ...
                $page->updatedAt,
                $page->pageType === 'project' ? 0.8 : 0.6,
                'monthly',
                $page->pageType === 'legal' ? 'WebPage' : 'Article'
            );
        }
    }
    ...
    private function renderHomepageList(string $message = '', string $variant = 'info'): void
    {
        ...
        $this->theme->start_admin_content(
            ...
            array_values(array_filter([
                $allows('pages.create')
                    ? ['label' => 'Dodaj sekcję', 'href' => 'index.php?route=/admin/homepage/create', 'variant' => 'primary']
                    : null,
                ['label' => 'Podgląd roboczy', 'href' => 'index.php?route=/admin/homepage/preview', 'variant' => 'outline-light'],
            ]))
        );
    ...
    private function renderHomepageForm(
        ...
        $value = static function (string $key, string $fallback = '') use ($values, $section): string {
            ...
            return match ($key) {
                ...
                'acrostic_words' => $section?->acrosticWords ?? $fallback,
        ...
        $this->theme->render_form(
            ...
            [
                ...
                [
                    ...
                    'label' => 'Nadtytuł',
                    'value' => $value('eyebrow'),
                    'help' => 'Krótka etykieta nad nagłówkiem, często używana jako kategoria lub numer sekcji. Motywy wykorzystują ją też jako zwięzłą etykietę nawigacji.',
                ],
                [
                    'name' => 'acrostic_words',
                    ...
                    'rows' => 6,
                    'maxlength' => 500,
                    'value' => $value('acrostic_words'),
                    'placeholder' => 'SYSTEM YIELDING NEXT-GEN TOOLS APPS X-PLATFORM',
                    'help' => 'Opcjonalne dla Hero + Split. Wyrazy oddziel spacją lub nową linią; każdy trafi do osobnego wiersza, a pierwsze litery utworzą pionowy akrostych.',
                ],
                [
                    'name' => 'section_type',
                    'label' => 'Typ sekcji',
                    ...
                    'cards' => $this->homepageSectionTypeCards(),
                    'help' => 'Hero jest pierwszym mocnym blokiem strony. Sekcja treści służy do opisów, kart i kolumn. CTA to krótki blok z wyraźnym przyciskiem akcji.',
                ],
                [
                    'name' => 'layout',
                    'label' => 'Układ',
                    ...
                    'cards' => $this->homepageLayoutCards(),
                    'help' => 'Pełna szerokość pokazuje jeden blok treści. Split dzieli nagłówek i treść. Kolumny używają elementów sekcji jako kart. Accent wyróżnia blok wizualnie. Contact grupuje kanały i osoby.',
                ],
                [
                    'name' => 'content_html',
                    'label' => 'Treść',
                ...
                [
                    'name' => 'button_label',
                    'label' => 'Etykiety przycisków',
                    'type' => 'textarea',
                    'rows' => 4,
                    'value' => $value('button_label'),
                    'help' => 'Jeden przycisk w linii, np. Zobacz projekty. Kolejność musi odpowiadać adresom poniżej.',
                ],
                [
                    'name' => 'button_url',
                    'label' => 'Adresy przycisków',
                    'type' => 'textarea',
                    'rows' => 4,
                    'value' => $value('button_url'),
                    'help' => 'Jeden adres w linii. Dozwolone: https://, http://, mailto:, kotwica # lub index.php. Puste pola oznaczają brak przycisków.',
                ],
                [
                    'name' => 'is_visible',
                    ...
                    'type' => 'checkbox',
                    'checked' => $visible,
                    'help' => 'Odznacz, aby pracować nad sekcją roboczo. Widok Podgląd roboczy pokazuje także ukryte sekcje administratorowi.',
                ],
            ],
    ...
    /** @return list<array{key: string, label: string, description: string, preview: string}> */
    private function homepageSectionTypeCards(): array
    {
        return [
            [
                'key' => 'hero',
                'label' => 'Hero / otwarcie strony',
                'description' => 'Najmocniejszy blok startowy, zwykle pierwszy ekran strony.',
                'preview' => 'hero',
            ],
            [
                'key' => 'content',
                'label' => 'Sekcja treści',
                'description' => 'Uniwersalny blok opisowy, techniczny albo informacyjny.',
                'preview' => 'content',
            ],
            [
                'key' => 'cta',
                'label' => 'Wezwanie do działania',
                'description' => 'Krótki komunikat kierujący użytkownika do kolejnego kroku.',
                'preview' => 'cta',
            ],
        ];
    }
    ...
    /** @return list<array{key: string, label: string, description: string, preview: string}> */
    private function homepageLayoutCards(): array
    {
        return [
            [
                'key' => 'full',
                'label' => 'Pełna szerokość',
                'description' => 'Jedna kolumna treści z nagłówkiem i opisem.',
                'preview' => 'full',
            ],
            [
                'key' => 'split',
                'label' => 'Nagłówek i treść obok siebie',
                'description' => 'Dwie kolumny, dobre do hero, akrostychu albo porównania.',
                'preview' => 'split',
            ],
            [
                'key' => 'columns',
                'label' => 'Treść w kolumnach',
                'description' => 'Karty lub elementy sekcji ustawione w responsywnej siatce.',
                'preview' => 'columns',
            ],
            [
                'key' => 'accent',
                'label' => 'Panel wyróżniony',
                'description' => 'Zwarty blok z mocniejszym tłem dla ważnej informacji.',
                'preview' => 'accent',
            ],
            [
                'key' => 'contact',
                'label' => 'Kontakt / kanały i osoby',
                'description' => 'Układ dla kanałów kontaktu i osób powiązanych z sekcją.',
                'preview' => 'contact',
            ],
        ];
    }
    ...
    private function validatedHomepageInput(Request $request, ?int $exceptId = null): array
    {
        ...
        $buttonLabels = $this->normalizeButtonLines($request->postString('button_label'), 120);
        $buttonUrls = $this->normalizeButtonLines($request->postString('button_url'), 500);
        $contentFormat = (new ContentRenderer())->normalizeFormat($request->postString('content_format'));
        $acrosticWords = preg_split(
            '/\s+/u',
            trim($request->postString('acrostic_words')),
            -1,
            PREG_SPLIT_NO_EMPTY
        ) ?: [];
        $normalizedAcrostic = implode("\n", $acrosticWords);
        $data = [
            'section_key' => $sectionKey,
            'section_type' => $sectionType,
            'eyebrow' => substr($request->postString('eyebrow'), 0, 160),
            'acrostic_words' => $normalizedAcrostic,
            'title' => $title,
        ...
        if (count($acrosticWords) > 12
            || strlen($normalizedAcrostic) > 500
            || array_any($acrosticWords, static fn (string $word): bool => strlen($word) > 60)) {
            return [$data, 'Akrostych może zawierać maksymalnie 12 wyrazów, po 60 znaków każdy.'];
        }
        $buttonError = $this->validateButtonPairs($buttonLabels, $buttonUrls);
        if ($buttonError !== '') {
     ...
     * @return list<string>
     */
    private function normalizeButtonLines(string $value, int $maxLength): array
    {
        ...
        foreach ($lines as $line) {
            ...
            $normalized[] = substr($line, 0, $maxLength);
        }
     ...
     */
    private function validateButtonPairs(array $labels, array $urls): string
    {
        if (count($labels) !== count($urls)) {
            return 'Każda etykieta przycisku musi mieć adres w tej samej linii.';
        }
        if (count($labels) > 5) {
            return 'Możesz dodać maksymalnie pięć przycisków.';
        }
        foreach ($urls as $url) {
            if (preg_match('~^(?:https?://|mailto:|#|index\.php(?:\?|$))~i', $url) !== 1) {
                return 'Adres przycisku używa niedozwolonego schematu.';
            }
        }
    ...
    private function renderHomepagePreview(): void
    {
        ...
        array_unshift($sections, [
            ...
            'acrostic_words' => '',
            'title' => 'Podgląd roboczy strony głównej',
            'content_html' => 'Ten widok pokazuje sekcje i elementy ukryte publicznie. Nie jest cacheowany, nie trafia do indeksu wyszukiwarek i służy wyłącznie do sprawdzania zmian przed publikacją.',
            'content_format' => ContentRenderer::HTML,
            'layout' => 'accent',
            'button_label' => 'Wróć do edycji',
            'button_url' => 'index.php?route=/admin/homepage',
            'items' => [],
        ]);
        $pages = array_map(
    ...
    private function renderHomepageItemForm(
        ...
        $this->theme->render_form(
            ...
            [
                ...
                ['name' => 'title', 'label' => 'Tytuł', 'value' => $value('title'), 'help' => 'Główny tytuł karty, kanału albo osoby widoczny w sekcji.'],
                [
                    'name' => 'content',
                    'label' => 'Opis',
                    'type' => 'richtext',
                ...
                [
                    ...
                    'help' => 'Karta to zwykły kafel treści. Kanał jest zoptymalizowany pod kontakt, np. Discord lub e-mail. Osoba używa układu profilu członka zespołu.',
                ],
                [
                    'name' => 'icon_key',
                    'label' => 'Ikona',
                ...
                [
                    ...
                    'rows' => 4,
                    'value' => $value('button_label'),
                    'help' => 'Jeden przycisk w linii. Przy powiązanej podstronie pierwszy wpis może służyć jako etykieta linku do tej strony.',
                ],
                [
                    'name' => 'button_url',
                    'label' => 'Adresy przycisków',
                    ...
                    'rows' => 4,
                    'value' => $value('button_url'),
                    'help' => 'Jeden adres w linii. Dozwolone: https://, http://, mailto:, kotwica # lub index.php. Jeśli wybierzesz powiązaną podstronę, ona ma pierwszeństwo dla pierwszego linku.',
                ],
                [
                    'name' => 'variant',
                    ...
                    'help' => 'Wariant wybiera akcent koloru karty. Primary jest najważniejszy, violet pomocniczy, neutral najspokojniejszy.',
                ],
                [
                    'name' => 'width',
                    'label' => 'Szerokość',
                    ...
                    'help' => 'Szeroka karta zajmuje więcej miejsca w układzie kolumnowym, jeśli aktywny motyw i szerokość ekranu na to pozwalają.',
                ],
                [
                    'name' => 'is_visible',
                    'label' => 'Element widoczny',
                    'type' => 'checkbox',
                    'checked' => $visible,
                    'help' => 'Ukryty element pozostaje w panelu i może być dopracowany przed publikacją.',
                ],
            ],
        ...
        $this->theme->render_form(
            ...
            [
                ...
                [
                    ...
                    'label' => 'Grafika z biblioteki',
                    'type' => 'select',
                    'value' => '',
                    'options' => $this->mediaOptions(),
                    'help' => 'Opcjonalnie dopisze wybraną grafikę do treści przy zapisie.',
                ],
                [
    ...
    private function validatedInput(Request $request, ?int $exceptId = null): array
    {
        ...
        $media = $this->selectedMediaAsset($request);
        if ($media instanceof MediaAsset) {
            $content = $this->appendMediaToContent($content, $contentFormat, $media);
        }
    ...
    {
        if (!$this->mediaAssets instanceof MediaAssetRepository) {
            return ['' => 'Biblioteka grafik niedostępna'];
        }

        return ['' => 'Nie dodawaj grafiki'] + $this->mediaAssets->options();
    }

    private function selectedMediaAsset(Request $request): ?MediaAsset
    {
        if (!$this->mediaAssets instanceof MediaAssetRepository) {
            return null;
        }

        return $this->mediaAssets->findByPublicPath($request->postString('content_media_path'));
    }

    private function appendMediaToContent(string $content, string $contentFormat, MediaAsset $asset): string
    {
        if (str_contains($content, $asset->publicPath)) {
            return $content;
        }
        $alt = $asset->altText !== '' ? $asset->altText : $asset->title;
        if ($contentFormat === ContentRenderer::MARKDOWN) {
            return rtrim($content) . "\n\n![" . str_replace([']', "\n", "\r"], ['', ' ', ' '], $alt) . '](' . $asset->publicPath . ")\n";
        }

        return rtrim($content) . "\n\n<p><img src=\"" . htmlspecialchars($asset->publicPath, ENT_QUOTES, 'UTF-8') . '" alt="' . htmlspecialchars($alt, ENT_QUOTES, 'UTF-8') . "\"></p>\n";
    }
    ...
    private function isIndexablePage(Page $page): bool
    {
        if ($page->slug === 'polityka-prywatnosci-aplikacji-medstock') {
            return false;
        }

        $content = trim(strip_tags($page->content));
        if ($page->pageType === 'project') {
            return strlen($content) >= 900;
        }

        return strlen($content) >= 1200;
    }
