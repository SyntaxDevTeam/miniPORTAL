...
final class Router
{
    ...
    public function dispatch(Request $request): int
    {
        ...
        $handler = $this->routes[$path][$effectiveMethod] ?? null;

        if ($handler !== null) {
            $this->runHandler($handler, $request, $method === 'HEAD');
            return 200;
        }
        ...
        foreach ($this->parameterizedRoutes as $route) {
            ...
            $this->runHandler($route['handler'], $request->withRouteParameters($parameters), $method === 'HEAD');
            return 200;
        }
    ...
    private function runHandler(Closure $handler, Request $request, bool $discardBody): void
    {
        if ($discardBody) {
            ob_start();
        }
