...
final class ModuleManagerService
{
     ...
     * @return list<string>
     */
    public function refresh(string $moduleId): array
    {
        $manifest = $this->manifest($moduleId);
        $this->assertLoadable($manifest);
        foreach ($manifest->requiredModules as $dependency) {
        ...
        return $this->installer->refresh($manifest);
    }
    ...
    public function exportPackage(string $moduleId): array
    {
        ...
        return $this->exportPackageTo($moduleId, $targetDirectory);
    }

    /**
     * @return array{path: string, filename: string, mime: string}
     */
    public function exportPackageTo(string $moduleId, string $targetDirectory): array
    {
    ...
    public function sourceFingerprint(string $moduleId): string
    {
        return (new ModulePackageExporter())->sourceFingerprint($this->manifest($moduleId));
    }
