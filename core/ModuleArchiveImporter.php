...
final class ModuleArchiveImporter
{
    public function __construct(
        ...
        private readonly int $maxUnpackedBytes = 52428800,
    ) {
    }
    ...
    /**
     * @param null|callable(ModuleManifest, string): void $updateInstalled
    ...
    ): array
    {
        ...
        $versionComparison = version_compare($manifest->version, $existingManifest->version);
        if ($versionComparison < 0) {
            throw new RuntimeException('Importowana aktualizacja nie może mieć niższej wersji niż bieżący kod modułu.');
        }
        ...
        try {
            ...
            @chmod($target, 0770);
            $approvedManifest = $this->validator->validate($target);
            $operation = $versionComparison === 0 ? 'refreshed' : 'updated';
            $updateInstalled($approvedManifest, $operation);
        } catch (\Throwable $exception) {
        ...
        return ['directory' => $target, 'manifest' => $approvedManifest, 'operation' => $operation];
    }
    ...
    private function extract(string $archive, string $target, string $extension): void
    {
        if ($extension === 'zip') {
            ...
            foreach (new \RecursiveIteratorIterator($phar) as $item) {
                ...
                $fileCount++;
                $this->assertArchiveEntryCount($fileCount);
    ...
    private function assertArchiveEntryCount(int $count): void
    {
        if ($count > $this->maxFiles) {
            throw new RuntimeException('Archiwum modułu zawiera zbyt wiele plików.');
        }
    }
    ...
    private function assertArchiveFileSize(int $bytes): void
    {
        if ($bytes > $this->maxFileBytes) {
            throw new RuntimeException('Archiwum modułu zawiera zbyt duży plik.');
        }
    }
    ...
    private function assertArchiveTotalSize(int $bytes): void
    {
        if ($bytes > $this->maxUnpackedBytes) {
            throw new RuntimeException('Archiwum modułu przekracza dozwolony rozmiar po rozpakowaniu.');
        }
    }
