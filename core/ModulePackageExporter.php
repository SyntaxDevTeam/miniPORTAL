...
final class ModulePackageExporter
{
    ...
    public function sourceFingerprint(ModuleManifest $manifest): string
    {
        $files = $this->exportableFiles($manifest->directory);
        $signatureFile = null;
        $manifestData = json_decode((string) file_get_contents($manifest->directory . '/info.json'), true);
        if (is_array($manifestData) && is_string($manifestData['signature'] ?? null)) {
            $signatureFile = $manifestData['signature'];
        }

        $context = hash_init('sha256');
        foreach ($files as $relative) {
            if ($signatureFile !== null && $relative === $signatureFile) {
                continue;
            }
            $checksum = hash_file('sha256', $manifest->directory . '/' . $relative);
            if (!is_string($checksum)) {
                throw new RuntimeException("Nie można obliczyć odcisku pliku {$relative}.");
            }
            hash_update($context, $relative . "\0" . $checksum . "\0");
        }
