...
document.querySelectorAll("[data-home-terminal]").forEach((terminal) => {
  ...
  const helpText = [
    ...
    "  status     miniPORTAL runtime status",
    "  ls         public site areas",
    "  cd <name>  open: projects, downloads, wiki, team or home",
  ...
  const execute = (rawCommand) => {
    ...
    if (name === "whoami") {
      appendLine(authenticated ? "authenticated-user" : "visitor (public session)");
      return;
    }
    if (name === "cd") {
