...
final class Theme implements ThemeInterface
{
    ...
    public function render_line_chart(array $points, string $label): void
    {
        ...
        echo '<figure class="line-chart" role="group" aria-label="' . $this->escape($label) . '"><div id="' . $chartId . '" class="amchart-host amchart-line" data-amchart="line" data-chart-label="' . $this->escape($label) . '" data-chart-payload="' . $this->escape($payload) . '" hidden></div><div class="chart-fallback">';
        echo '<svg viewBox="0 0 640 220" role="img" aria-labelledby="line-chart-title-' . substr(hash('sha256', $label), 0, 10) . '">';
    ...
    public function render_geo_map(array $points, string $label): void
    {
        ...
        echo '<figure class="geo-map" role="group" aria-label="' . $this->escape($label) . '"><div id="' . $chartId . '" class="amchart-host amchart-map" data-amchart="map" data-chart-label="' . $this->escape($label) . '" data-chart-payload="' . $this->escape($payload) . '" hidden></div>';
        echo '<div class="chart-fallback"><p>Interaktywna mapa wymaga załadowania biblioteki wykresów.</p><ul class="geo-map-fallback">';
        foreach (array_slice($points, 0, 20) as $point) {
            echo '<li><span>' . $this->escape((string) $point['label']) . '</span><strong>' . $this->escape((string) max(1, (int) $point['value'])) . '</strong></li>';
        }
