...
const faviconSourceIndex = process.argv.indexOf('--favicon-source');
const outputDirectoryIndex = process.argv.indexOf('--output-dir');
const appNameIndex = process.argv.indexOf('--app-name');
const themeColorIndex = process.argv.indexOf('--theme-color');
if (faviconSourceIndex !== -1 || outputDirectoryIndex !== -1) {
  if (faviconSourceIndex === -1 || outputDirectoryIndex === -1) {
    throw new Error('Tryb favicon wymaga --favicon-source i --output-dir.');
  }
  const customSource = decodePng(readFileSync(resolve(process.argv[faviconSourceIndex + 1])));
  const customBounds = alphaBounds(customSource);
  const customOutput = resolve(process.argv[outputDirectoryIndex + 1]);
  const appName = appNameIndex !== -1 ? process.argv[appNameIndex + 1].trim().slice(0, 80) : 'miniPORTAL';
  const requestedColor = themeColorIndex !== -1 ? process.argv[themeColorIndex + 1].trim().toLowerCase() : '#080c12';
  const themeColor = /^#[0-9a-f]{6}$/.test(requestedColor) ? requestedColor : '#080c12';
  ...
  const customVariants = new Map([
    ['favicon-16x16.png', renderSquare(customSource, customBounds, 16, 0.9, null)],
    ['favicon-32x32.png', renderSquare(customSource, customBounds, 32, 0.9, null)],
    ['favicon-48x48.png', renderSquare(customSource, customBounds, 48, 0.9, null)],
    ['favicon-64x64.png', renderSquare(customSource, customBounds, 64, 0.9, null)],
    ['favicon-96x96.png', renderSquare(customSource, customBounds, 96, 0.9, null)],
    ['favicon-128x128.png', renderSquare(customSource, customBounds, 128, 0.9, null)],
    ['favicon-256x256.png', renderSquare(customSource, customBounds, 256, 0.9, null)],
    ['apple-touch-icon.png', renderSquare(customSource, customBounds, 180, 0.82, themeBackground)],
    ['icon-192.png', renderSquare(customSource, customBounds, 192, 0.82, themeBackground)],
    ['icon-512.png', renderSquare(customSource, customBounds, 512, 0.82, themeBackground)],
    ['icon-maskable-512.png', renderSquare(customSource, customBounds, 512, 0.66, themeBackground)],
  ]);
  const customIco = encodeIco([16, 32, 48, 64, 128, 256].map(
    (size) => [size, customVariants.get(`favicon-${size}x${size}.png`)],
  ));
  mkdirSync(customOutput, { recursive: true });
  for (const [name, data] of customVariants) writeFileSync(join(customOutput, name), data);
  writeFileSync(join(customOutput, 'favicon.ico'), customIco);
  writeFileSync(join(customOutput, 'site.webmanifest'), JSON.stringify({
    name: appName || 'miniPORTAL',
    short_name: (appName || 'miniPORTAL').slice(0, 30),
    icons: [
